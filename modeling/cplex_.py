...
def populatebyrow(prob):
    ...
    lbs = prob.variables.get_lower_bounds() #get_lower_bounds: returns the lower bounds on variables from the problem.	
    # print(lbs)
    # ub1 is just the first lower bound
    ub1 = prob.variables.get_upper_bounds(1) # get_upper_bounds: returns the upper bounds on variables from the problem.
    # print(ub1)
...
def populatebynonzero(prob):
    ...
    prob.linear_constraints.add(rhs = my_rhs, senses = my_sense,names = my_rownames)
    prob.variables.add(obj = my_obj, ub = my_ub, names = my_colnames)
    # have 2 row, 3 col , and 6 value từ trái sang phải từ trên xuống 
    rows = [0,0,0,1,1,1]
    cols = [0,1,2,0,1,2]
...
def lpex1(pop_method):
    try:
        my_prob = cplex.Cplex()
        my_prob = my_prob.variables.add(types = [my_prob.variables.type.integer,my_prob.variables.type.integer.my_prob.variables.type.integer] )
        if pop_method == "r":
