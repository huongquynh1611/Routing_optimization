...
import numpy as np
def ex1():
    xls = pd.ExcelFile('C:\\Users\\tlhqu\\Desktop\\quynhxinh.xlsx')
    num_col=3
    num_row = 7
    try:
...
def ex2():
    ...
    my_sense = ["E"]*13 + ["L"]*4 + ["E"] * 36 + ["E"]*4 + ["L"] *40 + ["G"] *40 + ["E"]* len(names_7)  #+["L"]*360
    # print(len(my_sense))
    my_rhs = [1]*13 + [float(capity.iat[i]) for i in range(0,4)] + [0]*36 + [1]*4 + [float(end_time.iat[i]) for i in range(0,10)]*4 + [float(start_time.iat[i]) for i in range(0,10)]*4 + [0]*len(names_7)  #+ time_travel
